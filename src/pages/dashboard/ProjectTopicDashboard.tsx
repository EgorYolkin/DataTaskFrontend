                        ...
                        {topic.allowedUsers.map((user) => (
                            <div
                                ...
                                <Avatar className="h-8 w-8 rounded-lg">
                                    ...
                                    <AvatarFallback className="rounded-lg bg-gray-500">{user.name[0].toUpperCase()}{user.name[1].toUpperCase()}</AvatarFallback>
                                </Avatar>
...
export const InviteUserDialog: React.FC<InviteUserDialogProps> = ({projectID}) => {
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <div
                    className="flex items-center gap-2 cursor-pointer  bg-black text-white w-fit pr-3 pl-3 pt-1 pb-1 rounded-[10px]">
...
export const ProjectTopicDashboard: React.FC<ProjectDashboardProps> = ({
    ...
    const foundProjectAndTopic = useMemo(() => {
        ...
        let topic = project?.topics.find(
            (t) => t.name.toLowerCase() === topicName.toLowerCase()
        );

        // If not found in projects, search in sharedProjects
        if (!project) {
